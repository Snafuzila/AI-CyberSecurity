...
PORT_THRESHOLD = int(os.environ.get("NETWORK_SCAN_PORT_THRESHOLD", "4"))
...
def create_candidate_findings(
    ...
    for line_number, line in enumerate(lines, start=1):
        ...
        if "network scan detected" in normalized:
            # 1. Parse out the ports list from the log line
            port_count = 0
            if "against ports " in normalized:
                ports_part = line.split("against ports ", 1)[1].strip()
                # Split by commas and filter out any empty strings
                ports_list = [p.strip() for p in ports_part.split(",") if p.strip()]
                port_count = len(ports_list)
            
            # 2. Enforce the configured threshold
                ...
                findings.append(
                    {
                        ...
                        "detection_rule": (
                            f"The log contains 'network scan detected' with "
                            f"{port_count} ports scanned (Threshold: {PORT_THRESHOLD})."
                        ),
                        # Step 5 requirement: Add threshold to findings JSON
                        "network_scan_port_threshold": PORT_THRESHOLD 
                    }
                )
