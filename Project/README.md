...
## Research: XAI/RAI comparison (Isolation Forest vs. Autoencoder)

`research/xai_rai_comparison/` is a standalone offline suite (no imports from the live
Kafka pipeline above) that trains Isolation Forest and an Autoencoder on NSL-KDD,
compares their XAI evidence (SHAP vs. reconstruction error) and RAI track record
(precision/recall/F1/FPR/fairness gap), then sends each resulting SOC alert prompt to a
local Ollama model (default `llama3.1:8b`) to compare the two models' triage responses
side by side. Requires `pip install -e ".[xai-research]"` (adds `torch`, `shap`,
`scipy`) and the NSL-KDD files placed in `data/` — see
[research/xai_rai_comparison/README.md](research/xai_rai_comparison/README.md) for
